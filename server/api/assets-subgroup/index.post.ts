...
export default eventHandler(async (event) => {
        const { description, status } = await readBody<AssetsSubgroup>(event)
    
        const isAuthenticated = await authenticated(event)

        const result = AssetsSubgroup.safeParse({ description, status })
        ...
        const assetsSubgroupExists = await db
            .select()
            .from(tables.assetsSubgroup)
            .where(eq(tables.assetsSubgroup.description, description))
            .limit(1)
        if (assetsSubgroupExists.length > 0) {
