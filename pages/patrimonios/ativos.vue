...
const { loading, assets, deleteAssets } = useAssets()
...
function select(row: Assets) {
    const findStore = assets.value.find((f) => f.id === row.id)
...
function handleDelete(status: boolean) {
   if(status){
    deleteAssets(idDelete.value)
   }
}
function handleDeleteOpenModal(row: Assets) {
    if(row.id === '') return
    idDelete.value = row.id!
    description.value = 'Tem certeza que deseja deletar esse Ativo? ' + row.description
...
const filteredRows = computed(() => {
    ...
    if (!q.value) {
        return assets.value.slice((page.value - 1) * pageCount, (page.value) * pageCount)
    }
...
<template>
    ...
    <MyAssetsModal :data="selected" />
    <MyDeleteModal title="Deletar Ativo" :description="description" :fn="handleDelete" />
    <div>
        <UCard>
            <h3 class='px-3 text-2xl font-bold'>Ativos</h3>
            <div
