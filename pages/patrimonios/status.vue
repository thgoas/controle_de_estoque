...
const { loading, status, deleteStatus } = useStatus()
...
function select(row: Status) {
    const findStore = status.value.find((f) => f.id === row.id)
...
function handleDelete(status: boolean) {
   if(status){
    deleteStatus(idDelete.value)
   }
}
function handleDeleteOpenModal(row: Status) {
    if(row.id === '') return
...
const filteredRows = computed(() => {
    ...
    if (!q.value) {
        return status.value.slice((page.value - 1) * pageCount, (page.value) * pageCount)
    }
    return status.value.filter((f) => {
...
<template>
    ...
    <MyStatusModal :data="selected" />
    <MyDeleteModal title="Deletar Status" :description="description" :fn="handleDelete" />
    <div>
        <UCard>
            <h3 class='px-3 text-2xl font-bold'>Status</h3>
            <div
            ...
            <div class="flex justify-end px-3 py-3.5 border-t border-gray-200 dark:border-gray-700">
                <UPagination v-model="page" :page-count="pageCount" :total="status.length" />
            </div>
        </UCard>
    </div>
</template>
